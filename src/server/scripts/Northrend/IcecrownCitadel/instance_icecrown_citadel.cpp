                ...
                switch (entry)
                {
                    ...
                    case NPC_ZAFOD_BOOMBOX:
                        if (GameObjectTemplate const* go = sObjectMgr->GetGameObjectTemplate(GO_THE_SKYBREAKER_A))
                            if ((TeamInInstance == ALLIANCE && data->GetMapId() == go->moTransport.mapID) ||
                                (TeamInInstance == HORDE && data->GetMapId() != go->moTransport.mapID))
                                return entry;
                        return 0;
                    case NPC_IGB_MURADIN_BRONZEBEARD:
                        if ((TeamInInstance == ALLIANCE && data->GetPositionX() > 10.0f) ||
                            (TeamInInstance == HORDE && data->GetPositionX() < 10.0f))
                            return entry;
                        return 0;
                    default:
                        break;
